    ...
    public static class Utility
    {
		...
		*/
            ...
            bool onMinimap = false)
        {
            if (!onMinimap)
            {
				new Circle { Color = color, BorderWidth = thickness, Radius = radius }.Draw(center);
                return;
            }
        ...
        public static bool HasMastery(this AIHeroClient source, Mastery data)
        {
            return (source.IsValidTarget()) &&
                   source.Masteries.Find(m => m.Page == data.Page && m.Id == data.Id) != null;
        }
