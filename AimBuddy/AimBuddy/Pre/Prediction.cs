...
namespace SPrediction
{
    ...
    public static class Prediction
    {
        ...
        {
			public Vector3 QQPosition;
            public Vector2 CastPosition;
            public Vector2 UnitPosition;
            public AimBuddy.SDK.HitChance HitChance;
        ...
        public static void Initialize(Menu Menu)
        {
            ...
            if (Menu != null)
            {
				Chat.Print("Create Menu");
				predMenu = Menu.AddSubMenu("Prediction", "Prediction");
                predMenu.Add("PREDICTONLIST", new EloBuddy.SDK.Menu.Values.Slider("Prediction Method (1 = SPrediction, 2 = Common Predicion)", 1, 1, 2));
                predMenu.Add("SPREDWINDUP", new CheckBox("Check for target AA Windup", true));
                predMenu.Add("SPREDWPANALYSIS", new EloBuddy.SDK.Menu.Values.Slider("Waypoint analysis splitting method (1 = By target bounding radius, 2 = By spell width)", 2, 1, 2));
                predMenu.Add("SPREDMAXRANGEIGNORE", new EloBuddy.SDK.Menu.Values.Slider("Max Range Dodge Ignore (%)", 50, 0, 100));
                predMenu.Add("SPREDREACTIONDELAY", new EloBuddy.SDK.Menu.Values.Slider("MaxIgnore Rection Delay", 0, 0, 200));
                predMenu.Add("SPREDDELAY", new EloBuddy.SDK.Menu.Values.Slider("Spell Delay", 0, 0, 200));
                predMenu.Add("SPREDHC", new EloBuddy.SDK.Menu.Values.KeyBind("Count HitChance", false, EloBuddy.SDK.Menu.Values.KeyBind.BindTypes.HoldActive, 32));
                predMenu.Add("SPREDDRAWINGS", new CheckBox("Enable Drawings"));
				/*
			...
			Obj_AI_Base.OnBuffGain += Obj_AI_Hero_OnBuffAdd;
            CustomEvents.Unit.OnDash += Unit_OnDash;

            Drawing.OnDraw += Drawing_OnDraw;
            AIHeroClient.OnProcessSpellCast += Obj_AI_Hero_OnProcessSpellCast;
            //AIHeroClient.OnDamage += Obj_AI_Hero_OnDamage;
            blInitialized = true;
        }
        ...
        internal static Result GetPrediction(Obj_AI_Base target, float width, float delay, float missileSpeed, float range, bool collisionable, SkillshotType type, List<Vector2> path, float avgt, float movt, float avgp, Vector2 from, Vector2 rangeCheckFrom)
		{
            ...
            try
            {
                ...
                if (path.Count <= 1 && movt > 100 && (Environment.TickCount - PathTracker.EnemyInfo[target.NetworkId].LastAATick > 300 || !predMenu["SPREDWINDUP"].Cast<CheckBox>().CurrentValue)) //if target is not moving, easy to hit (and not aaing)
				{
					result.HitChance = AimBuddy.SDK.HitChance.VeryHigh;
					result.QQPosition = target.ServerPosition;
                    ...
                    if (collisionable && (result.CollisionResult.Objects.HasFlag(Collision.Flags.Minions) || result.CollisionResult.Objects.HasFlag(Collision.Flags.YasuoWall)))
                        result.HitChance = AimBuddy.SDK.HitChance.Collision;

                    if (from.Distance(result.CastPosition) > range - GetArrivalTime(from.Distance(result.CastPosition), delay, missileSpeed) * target.MoveSpeed * (100 - predMenu["SPREDMAXRANGEIGNORE"].Cast<EloBuddy.SDK.Menu.Values.Slider>().CurrentValue) / 100f)
                        result.HitChance = AimBuddy.SDK.HitChance.OutOfRange;
                ...
                if (target is AIHeroClient)
				{
					...
					if (/*Environment.TickCount - PathTracker.EnemyInfo[target.NetworkId].LastAATick < 300 && */predMenu["SPREDWINDUP"].Cast<CheckBox>().CurrentValue)
					{
						Chat.Print("2");
						...
						{
							result.HitChance = AimBuddy.SDK.HitChance.High;
                            result.CastPosition = target.ServerPosition.To2D();
							result.QQPosition = target.ServerPosition;
							result.UnitPosition = result.CastPosition;
							result.CollisionResult = Collision.GetCollisions(from, result.CastPosition, width, delay, missileSpeed);
                ...
                if (d >= (avgt - movt) * target.MoveSpeed && d >= avgp)
                    result.HitChance = AimBuddy.SDK.HitChance.Medium;
        ...
        internal static Result GetDashingPrediction(Obj_AI_Base target, float width, float delay, float missileSpeed, float range, bool collisionable, SkillshotType type, Vector2 from)
        {
            ...
            if (target.IsDashing())
            {
                ...
                if (dashInfo.IsBlink)
                {
                    result.HitChance = AimBuddy.SDK.HitChance.Impossible;
                    return result;
                }
                ...
                {
                    result.HitChance = AimBuddy.SDK.HitChance.Dashing;
                    result.CastPosition = target.ServerPosition.To2D();
                    result.UnitPosition = result.CastPosition;
                    result.CollisionResult = Collision.GetCollisions(from, result.CastPosition, width, delay, missileSpeed);
        ...
        internal static Result GetImmobilePrediction(Obj_AI_Base target, float width, float delay, float missileSpeed, float range, bool collisionable, SkillshotType type, Vector2 from)
        {
            ...
            {
                result.HitChance = AimBuddy.SDK.HitChance.Immobile;
                result.CollisionResult = Collision.GetCollisions(from, result.CastPosition, width, delay, missileSpeed);
        ...
        internal static AimBuddy.SDK.HitChance GetHitChance(float t, float avgt, float movt, float avgp)
        {
            if (avgp > 400)
            {
                if (movt > 50)
                {
                    if (avgt - movt >= t * 1.25f)
                        return AimBuddy.SDK.HitChance.High;
                    else if (avgt - movt >= t * 0.5f)
                        return AimBuddy.SDK.HitChance.Medium;
                    else
                        return AimBuddy.SDK.HitChance.Low;
                }
                else
                    return AimBuddy.SDK.HitChance.VeryHigh;
            }
            else
                return AimBuddy.SDK.HitChance.High;
        }
        ...
        internal static Result WaypointAnlysis(Obj_AI_Base target, float width, float delay, float missileSpeed, float range, bool collisionable, SkillshotType type, List<Vector2> path, float avgt, float movt, float avgp, Vector2 from, float moveSpeed = 0, bool isDash = false)
        {
            ...
            if (pathBounds[0] != -1 && pathBounds[1] != -1)
            {
                for (int k = pathBounds[0]; k <= pathBounds[1]; k++)
                {
                    ...
                    if (predMenu["SPREDWPANALYSIS"].Cast<EloBuddy.SDK.Menu.Values.Slider>().CurrentValue == 1)
        ...
        public static int IgnoreReactionDelay
        {
            get
            {
                if (predMenu == null)
                    return 0;
                return predMenu["SPREDREACTIONDELAY"].Cast<EloBuddy.SDK.Menu.Values.Slider>().CurrentValue;
            }
        }
        ...
        public static int SpellDelay
        {
            get
            {
                if (predMenu == null)
                    return 0;
                return predMenu["SPREDDELAY"].Cast<EloBuddy.SDK.Menu.Values.Slider>().CurrentValue;
            }
        }
        ...
        private static void Unit_OnDash(Obj_AI_Base sender, AimBuddy.SDK.Dash.DashItem args)
        {
            
        }

        private static void Obj_AI_Hero_OnBuffAdd(Obj_AI_Base sender, Obj_AI_BaseBuffGainEventArgs args)
        {
        ...
        {
            if (predMenu != null && predMenu["SPREDDRAWINGS"].Cast<CheckBox>().CurrentValue)
            {
                ...
                Drawing.DrawText(Drawing.Width - 200, 60, System.Drawing.Color.Red, String.Format("Casted Spell Count: {0}", castCount));
                Drawing.DrawText(Drawing.Width - 200, 80, System.Drawing.Color.Red, String.Format("Hit Spell Count: {0}", hitCount));
                Drawing.DrawText(Drawing.Width - 200, 100, System.Drawing.Color.Red, String.Format("Hitchance (%): {0}%", castCount > 0 ? (((float)hitCount / castCount) * 100).ToString("00.00") : "n/a"));
            }
        }
        ...
        */
        private static void Obj_AI_Hero_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            {
                LastSpells.RemoveAll(p => Environment.TickCount - p.tick > 2000);
                if (sender.IsMe && !args.SData.IsAutoAttack() && predMenu["SPREDHC"].Cast<EloBuddy.SDK.Menu.Values.KeyBind>().CurrentValue)
                {
