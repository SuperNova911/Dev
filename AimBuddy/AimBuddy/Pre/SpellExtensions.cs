...
namespace SPrediction
{
    ...
    public static class SpellExtensions
    {
		...
		public static Vector3 GetSPosition(this Spell s, AIHeroClient target)
		{
			var result = LinePrediction.GetPrediction(target, s.Width, s.Delay, s.Speed, s.Range, s.Collision, target.GetWaypoints(), target.AvgMovChangeTime(), target.LastMovChangeTime(), target.AvgPathLenght(), s.From.To2D(), s.RangeCheckFrom.To2D());

			return result.QQPosition;
		}
		...
		*/
        ...
        public static bool SPredictionCast(this Spell s, AIHeroClient t, HitChance hc, int reactionIgnoreDelay = 0, byte minHit = 1, Vector3? rangeCheckFrom = null, float filterHPPercent = 100)
        {
            ...
            if (Prediction.predMenu != null && Prediction.predMenu["PREDICTONLIST"].Cast<EloBuddy.SDK.Menu.Values.Slider>().CurrentValue == 2)
            {
      ...
      /*  public static bool SPredictionCastArc(this Spell s, AIHeroClient t, HitChance hc, int reactionIgnoreDelay = 0, byte minHit = 1, Vector3? rangeCheckFrom = null, float filterHPPercent = 100)
        {
            if (Prediction.predMenu
                != null && Prediction.predMenu["PREDICTONLIST"].Cast<EloBuddy.SDK.Menu.Values.Slider>().CurrentValue == 2)
